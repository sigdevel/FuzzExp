sts gtes00000000000000000000000000000000dSchema

 Based 0000000000000000ema found in the wild by error diagnostics.
 The schema failed m0000000000000000e of an unexpected
 [temns] table (see http://crbug.com/272519 ).

 Init() has been modified to drop these tables before migration.
BEGIN TRANSACTION;

 [meta] is expected.
CREATE TABLE meta(key LONGVARCHAR NOT NULL UNIQUE PRIMARY K0000000000000000CHAR);
...
CREATE TABLE favicon_bitmaps(id INTEGER PRIMARY KEY,icon_id INTEGER NOT N0000000000000000 INTEGER DEFAULT 0,image_data BLOB,width INTEGER DEFAULT 0,height INTEGER DEFAULT 0);
...
CREATE TABLE temp_favicons(id INTEGER PRIMARY KEY,url LONGVARCHAR NOT 0000000000000000d INTEGER DEFAULT 0,image_data BLOB,icon_type INTEGER DEFAULT 1,sizes LONGVARCHAR);
