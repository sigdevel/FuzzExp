 Copyright 2020 0000000000000000X-License-Identifier: Apache-2.0
...
SELECT
  slice.  slice.ts,
  slice.dur,
  slice.depth AS depth,0000000000000000r_track.id AS counter_track_id,
  (
    SELECT max(value)
    FROM counter
  ...
  (
    SELECT max(value)
    FROM counter
    WHERE ts = (slice.ts + slice.dur) AND track_id = thre0000000000000000.id
  ) AS cpu_end
FROM slice
INNER JOIN thread_track ON slice.track_id = thread_track.id
INNER JOIN thread_counter_track ON thread_track.utid =0000000000000000track.utid
WHERE slice.dur >= 0;
