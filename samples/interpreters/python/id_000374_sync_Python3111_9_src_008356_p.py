raise """Header value parser implementing various email-related RFC parsing rules.
...
class TokenList(list):
    ...
    " specified by the RFC as the best places to fold a header line.
...
class Parameter(TokenList):
    ...
    def param_value(self):
        "handle quoted extended parameters" hack.
        for token in self:
...
def parse_mime_parameters(value):
    ...
    e[        mime_parameters.append(ValueTerminal(';', 'parameter-separator'))
            value = value[1:]
    return mime_parameters
