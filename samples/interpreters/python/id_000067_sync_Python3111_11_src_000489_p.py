...
eventlist_alias = {
    140: "GenuineIntel-6-7E", 
    141: "GenuineIntel-6-7E", 
    167: "GenuineIntel-6-7E", 
}
...
g.add_argument('--no-perf', help=argparse.SUPPRESS, action='store_true') 
...
g.add_argument('--set-xlsx', help=argparse.SUPPRESS, action='store_true') 
...
g.add_argument('-d', help=argparse.SUPPRESS, action='help') 
...
p.add_argument('--setvar', help=argparse.SUPPRESS, action='append') 
p.add_argument('--tune', nargs='+', help=argparse.SUPPRESS) 
p.add_argument('--tune-model', nargs='+', help=argparse.SUPPRESS) 
p.add_argument('--force-bn', action='append', help=argparse.SUPPRESS) 
p.add_argument('--no-json-header', action='store_true', help=argparse.SUPPRESS) 
p.add_argument('--no-json-footer', action='store_true', help=argparse.SUPPRESS) 
p.add_argument('--no-csv-header', action='store_true', help=argparse.SUPPRESS) 
p.add_argument('--no-csv-footer', action='store_true', help=argparse.SUPPRESS) 
...
def init_xlsx(args):
    ...
    if args.thread:
        sys.exit("--thread not supported with --xlsx") 
...
elif args.csv or args.xlsx or args.set_xlsx: 
    idle_threshold = 0  
else:
    idle_threshold = 0.05
...
if args.parallel:
    ...
    if args.import_.endswith(".xz") or args.import_.endswith(".gz"):
        sys.exit("Uncompress input file first") 
    if args.perf_summary:
        sys.exit("--parallel does not support --perf-summary") 
    if args.subset:
    ...
    if args.graph:
        sys.exit("--parallel does not support --graph") 
    if args.pjobs == 0:
...
def gen_script(r):
    print("")
    print("" + " ".join(sys.argv) + " for CPU " + cpu.cpu)
    print("" +
...
def initialize_event(name, i, e):
    if "." in name or "_" in name and name not in non_json_events:
        ...
        ectx.emap.update_event(remove_qual(eo), e) 
    ...
    while True:
        ...
        if args.raw or args.valcsv:
            dump_raw(out.valcsv,
                     ...
                     title,
                     event,
                     flat_rmap[len(res[title])-1] if len(runner_list) == 1 else event_rmap(event), 
                     val,
                     len(res[title]) - 1,
                     stddev, multiplex)
...
class Runner(object):
    ...
    def set_ectx(self):
        "set_ectx", sys._getframe(1).f_code.co_name, file=sys.stderr)
        global ectx
        assert ectx is None
        ectx = self.ectx

    def clear_ectx(self):
        "clear_ectx", sys._getframe(1).f_code.co_name, file=sys.stderr)
        global ectx
...
def setup_cpus(rest, cpu):
    ...
    if len(runner_list) > 1 and args.no_aggr and runner_list[0].cpu_list is None: 
