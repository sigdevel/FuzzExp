...
class CleanUp(object):
    ...
    def __init__(self, newFiles, modifiedFiles, newDirs = [], qSaveFiles=False, qCleanup=True):
        ...
        self.copies = []
        assert(types.ListType == type(newDirs)) 
...
class TestCommandLine(unittest.TestCase):
    ...
    def test_justPrepare(self):    
        ...
        self.assertTrue(len(list(glob.glob(self.currentResultsDir + "*"))), 1) "
        self.test_passed = True         
...
"Only performing quick tests")       

"ExpectedOutput/Orthobench_blast/"
"Input/Orthobench_blast/Orthogroups.csv"

"Input/Orthobench_blast/" + x for x in "OrthoFinder_v0.4.0_graph.txt clusters_OrthoFinder_v0.4.0_I1.5.txt clusters_OrthoFinder_v0.4.0_I1.5.txt_id_pairs.txt".split()]

"-b %sInput/Orthobench_blast" % baseDir)
    ...
    def test_DistanceMatrixEvalues(self):
        ...
        with open(outFN, 'rb') as infile:
            ...
            self.assertEqual('0.000000', line[1]) 
            self.assertEqual('0.000001', line[2]) 
            line = infile.next().rstrip().split()
...
"Inferring multiple sequence alignments for species tree" in self.stdout)
...
"Inferring multiple sequence alignments for species tree" in self.stdout)   
    ...
    def TestResolve(self, case):    
        ...
        with CleanUp([expectedResultfn], [], []):        
            subprocess.call("%s -s dash %s" % (resolve_script, infn), shell=True, env=my_env)   
    ...
    def CheckStandardRun(self, stdout, stderr, goldResultsDir, expectedCSVFileLocation):
        ...
        self.assertTrue("Connected putatitive homologs" in stdout) 
