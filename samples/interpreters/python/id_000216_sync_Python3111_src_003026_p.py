...
"Exp0.9").set_name("SA", register=True)
...
class ChoiceBase(base.Optimizer):
    ...
    def __init__(
        ...
        int_layers = [x for x in int_layers if x.arity is not None]  "Choice" instances for now
...
class NoisyBandit(base.Optimizer):
    ...
    Infinite arms: we add one arm when `20 * 
    """
...
"tenth", "third"]
"discretememeticT", register=True)
