...
TAIL_UNUSED_DEFAULT = 0  
TAIL_USED_DEFAULT = 10  
VERIFY_UNUSED_DEFAULT = None  
VERIFY_USED_DEFAULT = EMOJI  
VERSION_UNUSED_DEFAULT = None  
VERSION_USED_DEFAULT = PRINT  
SET_DEVICE_NAME_UNUSED_DEFAULT = None  
SET_DISPLAY_NAME_UNUSED_DEFAULT = None  
NO_SSL_UNUSED_DEFAULT = None  
SSL_CERTIFICATE_DEFAULT = None  
MXC_ID_PLACEHOLDER = "__mxc_id__"
HOMESERVER_PLACEHOLDER = "__homeserver__"  
...
OUTPUT_JSON = "json"
"json" but with transport_response object added
...
class GlobalState:
    ...
    def __init__(self):
        ...
        self.pa: argparse.Namespace = None  
        ...
        self.room_action = False  
        self.set_action = False  
        self.get_action = False  
        self.setget_action = False  
        self.err_count = 0  
        self.warn_count = 0  


"", useful when reporting values to stdout
...
def obj_to_dict(obj):
    ...
    if get_qualifiedclassname(obj) == "aiohttp.tracing.TraceConfig":
        return {obj.__class__.__name__: str(obj)}
    "keys must be str, int, float, bool or None" errors
    ...
    if hasattr(obj, "__dict__"):
        ...
        "Object {obj} ({type(obj)}) has no class dictionary. "
    ...
    async def message_callback(self, room: MatrixRoom, event):  
        ...
        try:
            ...
            if isinstance(event, RoomMessageMedia):  
                mxc = event.url  
                url = await self.client.mxc_to_http(mxc)  
...
def notify(title: str, content: str, image_url: str):
    ...
    try:
        if image_url:
            ...
            "notification-message-IM" will work on Ubuntu
...
def create_pid_file() -> None:
    ...
    try:
        ...
        with open(PID_FILE_DEFAULT, "w") as f:  
            f.write(str(pid))
            f.close()
        gs.log.debug(
...
def determine_store_dir() -> str:
    ...
    if not gs.pa.encrypted:
        return None
    pargs_store_norm = os.path.normpath(gs.pa.store)  
...
async def map_roomalias_to_roomid(client, alias) -> str:
    ...
    ---------
    client : nio client
    alias : can be an alias in the form of '
...
def is_room_alias(room_id: str) -> bool:
    ...
    if (
        ...
        and room_id.count(":") == 1
        and (" " not in room_id)
        and not any(elem in room_id for elem in "[]{} ")  
    ):
        return True
    else:
        return False
...
async def send_event(client, rooms, event):  
    """Process event.
    ...
    if event == "-":  
        jsondata = sys.stdin.buffer.read().decode()  
    else:
...
async def send_file(client, rooms, file):  
    ...
    "txt", "pdf", "mp3", "ogg", "wav", ...
    "^.pdf$|^.txt$|^.doc$|^.xls$|^.mobi$|^.mp3$",
    
    "File {file} is not a permitted file type. Should be "
    ".pdf, .txt, .doc, .xls, .mobi or .mp3 ... "
    "[{os.path.splitext(file)[1].lower()}]"
    "This file is being dropped and NOT sent.")
    

    "plain/text" "audio/ogg"
    mime_type = magic.from_file(file, mime=True)
    "application/")) and
    "plain/")) and
    "audio/"))):
    "File {file} does not have an accepted mime type. "
    "Should be something like application/pdf. "
    "Found mime type {mime_type}. "
    "This file is being dropped and NOT sent.")
    ...
    try:
        for room_id in rooms:
            ...
            if gs.pa.print_event_id:
                ...
                json_max = resp.__dict__
                json_max.update({"message": message})  
                json_ = json_max.copy()
                json_.pop("transport_response")
                json_spec = None
                print_output(
...
async def stream_messages_from_pipe(client, rooms):
    ...
    """
    stdin_ready = select.select([sys.stdin,], [], [], 0.0)[  
        0
    ]  
    if not stdin_ready:
        gs.log.debug(
...
async def login_using_credentials_file(
    ...
    if gs.pa.debug > 0:
        ...
        "Logged_in()={client.logged_in}") is always True.
...
async def listen_tail(  
    ...
    resp_s = await synchronize(client)  
    ...
    for room_id in rooms:  
        ...
        for event in resp.chunk:
            ...
            if client.rooms and client.rooms[room_id]:
                ...
                room = MatrixRoom(room_id, None, True)  
            await callbacks.message_callback(room, event)
        if resp.chunk:  
    ...
    for filename in gs.pa.upload:
        ...
        if isinstance(resp, UploadError):
            ...
            gs.log.debug(
                ...
                "'*** hidden to prevent leaks'"  "{decryption_dict}"
            )
...
async def action_delete_mxc(client: AsyncClient, credentials: dict) -> None:
    ...
    "https://SERVERHERE/_synapse/admin/v1/media/SERVERHERE/
    "
    for mxc in gs.pa.delete_mxc:
        ...
        srv_full = credentials["homeserver"]  
        srv_host = urlparse(srv_full).hostname  
        rest = (
            srv_full
        ...
        connector = TCPConnector(ssl=gs.ssl)  
...
async def action_joined_members(
    ...
    if "*" in rooms:
        ...
        if isinstance(resp, JoinedRoomsError):
            ...
            rooms = filter(lambda val: val != "*", rooms)  
        else:
            gs.log.debug(
                f"joined_rooms successful with {privacy_filter(str(resp))}"
            )
...
async def action_mxc_to_http(client: AsyncClient, credentials: dict) -> None:
    ...
    for mxc in gs.pa.mxc_to_http:
        mxc = mxc.strip()
        http = await client.mxc_to_http(mxc)  
...
async def action_get_avatar(client: AsyncClient, credentials: dict) -> None:
    ...
    if gs.pa.get_avatar == []:
        gs.pa.get_avatar.append(credentials["user_id"])  
    ...
    for user_id in gs.pa.get_avatar:
        ...
        if isinstance(resp, ProfileGetAvatarResponse):
            ...
            json_max = resp.__dict__
            json_max.update({"avatar_http": avatar_url})  
            json_ = json_max.copy()
            json_.pop("transport_response")
            json_spec = None
            print_output(
...
async def action_get_profile(client: AsyncClient, credentials: dict) -> None:
    ...
    if gs.pa.get_profile == []:
        gs.pa.get_profile.append(credentials["user_id"])  
...
async def action_get_openid_token(
    ...
    if gs.pa.get_openid_token == []:
        gs.pa.get_openid_token.append(credentials["user_id"])  
    ...
    for user_id in gs.pa.get_openid_token:
        ...
        if isinstance(resp, GetOpenIDTokenError):
            ...
            json_max = resp.__dict__
            json_max.update({"user_id": user_id})  
            json_ = json_max.copy()
            json_.pop("transport_response")
            json_spec = None
            print_output(
...
async def action_room_get_visibility(
    ...
    if gs.pa.room_get_visibility == []:
        gs.pa.room_get_visibility.append(credentials["room_id"])  
...
async def action_room_get_state(
    ...
    if gs.pa.room_get_state == []:
        gs.pa.room_get_state.append(credentials["room_id"])  
...
async def action_roomsetget() -> None:
    ...
    try:
        ...
        if gs.pa.room_get_visibility is not None:  
            await action_room_get_visibility(gs.client, gs.credentials)
        if gs.pa.room_get_state is not None:  
        ...
        if gs.pa.get_avatar is not None:  
            await action_get_avatar(gs.client, gs.credentials)
        if gs.pa.get_profile is not None:  
        ...
        if gs.pa.get_openid_token is not None:  
...
async def action_send() -> None:
    ...
    try:
        ...
        "gs.client.rooms are: {gs.client.rooms}")
...
async def action_login() -> None:
    ...
    homeserver_short = urlparse(homeserver).hostname  
    ...
    if user_id is not None:
        if is_partial_user_id(user_id):
            user_id = user_id + ":" + homeserver_short  
    ...
    if is_short_room_alias(room_id):
        if room_id[0] != "":
            room_id = "" + room_id
        room_id = room_id + ":" + homeserver_short  
...
def initial_check_of_args() -> None:  
    ...
    if (
        ...
        or gs.pa.room_get_visibility is not None  
        or gs.pa.room_get_state is not None  
    ...
    if (
        ...
        or gs.pa.get_avatar is not None  
        or gs.pa.get_profile is not None  
        or gs.pa.get_room_info is not None  
        or gs.pa.get_client_info
        or gs.pa.has_permission
        or gs.pa.export_keys
        or gs.pa.get_openid_token is not None  
        or gs.pa.whoami
    ):
        gs.get_action = True
    else:
...
def main_inner(
    ...
    ap.add_argument(
        "-u" has been moved to --user!
        "--download-media",
        type=str,
        default="",  
        action="store",
        nargs="?",  
        const=MEDIA_DIR_DEFAULT,  
    ...
    ap.add_argument(
        "-x", starting v2.21 -x is no longer supported
        "--set-device-name",
        required=False,
        type=str,
        default=SET_DEVICE_NAME_UNUSED_DEFAULT,  
        metavar="DEVICE_NAME",
    ...
    ap.add_argument(
        ...
        required=False,
        type=str,
        default=SET_DISPLAY_NAME_UNUSED_DEFAULT,  
    ...
    ap.add_argument(
        ...
        "See https://spec.matrix.org/v1.2/client-server-api/"
        ".",
    ...
    ap.add_argument(
        ...
        "it will be automatically prefixed with '"
    ...
    ap.add_argument(
        ...
        required=False,
        type=str,
        default=SSL_CERTIFICATE_DEFAULT,  
    ...
    ap.add_argument(
        ...
        required=False,
        type=str,
        default=VERSION_UNUSED_DEFAULT,  
        nargs="?",  
    ...
    if sys.stdout.isatty():
        ...
        term_width = 80
        "not in terminal, using default terminal width ", term_width)
        con = ""
        coff = ""
        eon = ""
        eoff = ""
...
""".replace(
    ...
    if gs.pa.readme:
        ...
        try:
            ...
            "BrokenPipeError caught", file=sys.stderr)
            pass
        return 0
