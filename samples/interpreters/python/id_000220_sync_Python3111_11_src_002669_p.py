...
def make_linux(
    ...
    package_root = sdist_leaf[ : -len('.tar.gz')]   
    ...
    def docker_system(command, prefix='', return_output=False):
        ...
        "function python3 {{ python3.9 \\$* ; }} ; {command}"'
...
def parse_remote(remote):
    ...
    '''
    re_user_at = '(?P<user_at>[^@]+@)'                          
    re_host = '(?P<host>[^:@]+)'                                
    re_user_host = f'(?P<user_at_host>{re_user_at}?{re_host})'  
    re_directory = '(:(?P<directory_name>[^:,]*))'              
    re_sync_directories = '(?P<directories>(,[^,:]*)*)'         
    re_ssh_extra = '((?P<ssh_extra>ssh [^@]+) )'                
    re_port = '(:(?P<port>[0-9]+))'                             
    re_jump = f'((?P<jump>{re_user_at}{re_host}{re_port}?)::)'  
