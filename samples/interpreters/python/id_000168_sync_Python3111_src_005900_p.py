...
"SIM Hit ground", timeout=timeout)
        self.wait_disarmed()
    ...
    def AutoTuneSwitch(self):
        ...
        "customising" the commandline here:
    ...
    def MotorTest(self, timeout=60):
        ...
        pwm_in = 1300
        "+" - start motor of 2 is "B", which is
    ...
    def Landing(self):
        ...
        check_landing_speeds(
            ...
            self.get_parameter("LAND_SPEED") / 100,  
            self.get_parameter("LAND_ALT_LOW") / 100 
        )

        def test_landing_speeds(land_speed_high, land_speed_low, land_alt_low, **kwargs):
            self.set_parameters({
                "LAND_SPEED_HIGH": land_speed_high * 100,  
                "LAND_SPEED": land_speed_low * 100,  
                "LAND_ALT_LOW": land_alt_low * 100,  
            })
    ...
    def precision_loiter_to_pos(self, x, y, z, timeout=40):
        ...
        while True:
            ...
            "dest_ned=%s" % str(dest_ned))
            "pos_ned=%s" % str(pos_ned))
            delta_ef = dest_ned - pos_ned
            "delta_ef=%s" % str(delta_ef))
            ...
            "ATTITUDE"]
            "r=%f p=%f y=%f" % (math.degrees(att.roll), math.degrees(att.pitch), math.degrees(att.yaw)))
            "angle_x=%s angle_y=%s" % (str(math.degrees(angle_x)), str(math.degrees(angle_y))))
            "distance=%s" % str(distance))
    ...
    def OBSTACLE_DISTANCE_3D_test_angle(self, angle):
        ...
        while True:
            ...
            if m.orientation != expected_orientation:
                "Wrong orientation (want=%u got=%u)" %
                
                continue
            if abs(m.current_distance - expected_distance_cm) > 1:
                "Wrong distance (want=%f got=%f)" %
    ...
    def ProximitySensors(self):
        ...
        "magic" location SITL understands which
