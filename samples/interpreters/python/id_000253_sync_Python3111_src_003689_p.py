...
async def taskGetUrl(functionId, body, cookie):
    ...
    for n in range(5):
        ...
        try:
            ...
            "❌ 第{e.__traceback__.tb_lineno}行：{e}"
    ...
    if res['code'] == 0:
        if res['success']:
            ...
            for e, roundList_n in enumerate(roundList):               
                ...
                cashAmount = roundList_n['cashAmount']
                leftAmount = roundList_n['leftAmount']                
...
async def happyDigExchange(cookie):
    for n in range(1, 4):
        ...
        "已领取极速版红包 {res['data']['redValue']} 🧧")
        
        
        "0":
        
        "待提现微信现金 {res['data']['wxValue']} 💰")
...
async def spring_reward_list(cookie):
    ...
    if res['code'] == 0:
        if res['success']:
            ...
            for _items in items:
                amount = _items['amount']         
                prizeDesc = _items['prizeDesc']   
                prizeType = _items['prizeType']   
                amountid = _items['id']           
