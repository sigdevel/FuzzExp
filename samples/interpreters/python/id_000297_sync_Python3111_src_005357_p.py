...
def arbitration_id_converter(source):  
...
class Signal(object):
    ...
    * _multiplex ('Multiplexor' or Number of Multiplex)
    ...
    start_bit = attr.ib(default=0)  
    size = attr.ib(default=0)  
    is_little_endian = attr.ib(default=True)  
    is_signed = attr.ib(default=True)  
    ...
    receivers = attr.ib(factory=list)  
    comment = attr.ib(default=None)  
    multiplex = attr.ib(default=None)  

    mux_value = attr.ib(default=None)
    is_float = attr.ib(default=False)  
    is_ascii = attr.ib(default=False)  
    type_label = attr.ib(default="")
    enumeration = attr.ib(default=None)  
    comments = attr.ib(factory=dict)  
    attributes = attr.ib(factory=dict)  
    values = attr.ib(converter=normalize_value_table, factory=dict)  
    mux_val_grp = attr.ib(factory=list)  
    muxer_for_signal = attr.ib(default=None)  

    
    calc_min_for_none = attr.ib(default=True)  
    calc_max_for_none = attr.ib(default=True)  

    cycle_time = attr.ib(default=0)  
    initial_value = attr.ib(converter=float_factory, default=float_factory(0.0))  
...
class ArbitrationId(object):
    ...
    extended = attr.ib(default=False)  
    ...
    def pgn(self, value):  
        ...
        _pgn = value & 0x3FFFF
        self.id &= 0xfc0000ff
        self.id |= (_pgn << 8 & 0x3FFFF00)  



    @property
    def j1939_tuple(self):  
...
class Pdu(object):
    ...
    triggering_name = attr.ib(default="")  
    pdu_type = attr.ib(default="")  
    port_type = attr.ib(default="")  
    signals = attr.ib(factory=list)  
    signalGroups = attr.ib(factory=list)  
    cycle_time = attr.ib(default=0)  
...
class Frame(object):
    ...
    * comment
    ...
    arbitration_id = attr.ib(converter=arbitration_id_converter, default=0)  
    size = attr.ib(default=0)  
    transmitters = attr.ib(factory=list)  
    
    is_complex_multiplexed = attr.ib(default=False)  
    is_fd = attr.ib(default=False)  
    comment = attr.ib(default="")  
    signals = attr.ib(factory=list)  
    mux_names = attr.ib(factory=dict)  
    attributes = attr.ib(factory=dict)  
    receivers = attr.ib(factory=list)  
    signalGroups = attr.ib(factory=list)  
    ...
    is_j1939 = attr.ib(default=False)  
    
    

    pdus = attr.ib(factory=list)  
    header_id = attr.ib(default=None)  
    
    pdu_name = attr.ib(default="")  
    ...
    @property
    def get_multiplexer_values(self):  
    ...
    @property
    def is_pdu_container(self):  
        return len(self.pdus) > 0

    @property
    def get_pdu_id_values(self):  
...
class CanMatrix(object):
    ...
    type = attr.ib(default=matrix_class.CAN)  
    attributes = attr.ib(factory=dict)  
    ecus = attr.ib(factory=list)  
    frames = attr.ib(factory=list)  

    signal_defines = attr.ib(factory=dict)  
    frame_defines = attr.ib(factory=dict)  
    global_defines = attr.ib(factory=dict)  
    env_defines = attr.ib(factory=dict)  
    ecu_defines = attr.ib(factory=dict)  
    value_tables = attr.ib(factory=dict)  
    env_vars = attr.ib(factory=dict)  
    signals = attr.ib(factory=list)  

    baudrate = attr.ib(default=0)  
    fd_baudrate = attr.ib(default=0)  
    ...
    def add_frame_defines(self, name, definition):  
        """
    ...
    def add_ecu_defines(self, name, definition):  
        """
    ...
    def add_env_defines(self, name, definition):  
        """
    ...
    def add_global_defines(self, name, definition):  
        """
    ...
    def del_signal_attributes(self, unwanted_attributes):  
    ...
    def del_frame_attributes(self, unwanted_attributes):  
    ...
    def rename_ecu(self, ecu_or_name, new_name):  
    ...
    def rename_frame(self, frame_or_name, new_name):  
        """Rename Frame.
    ...
    def del_frame(self, frame_or_name):  
    ...
    def rename_signal(self, signal_or_name, new_name):  
        """Rename Signal.
