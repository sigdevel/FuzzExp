...
jsonDocumentTypes = (
        ...
        "https://xbrl.org/((~status_date_uri~))/xbrl-json" "template" test cases without CI production
    )
csvDocumentTypes = (
        "https://xbrl.org/2021/xbrl-csv",
        "http://www.xbrl.org/WGWD/YYYY-MM-DD/xbrl-csv",
        "http://xbrl.org/YYYY/xbrl-csv",
        "https://xbrl.org/((~status_date_uri~))/xbrl-csv" "template" test cases without CI production
    )
...
JSONNSReservedURIAliases = {} "/xbrl-json/error", "xbrlje") for ns in nsOims)
CSVNSReservedURIAliases = dict((ns + "/xbrl-csv/error", "xbrlce") for ns in nsOims)
NSReservedAliasURIPrefixes = { 
...
PrefixedQName = re_compile(
    ...
    "[_A-Za-z\xC0-\xD6\xD8-\xF6\xF8-\xFF\u0100-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD]"
     r"[_\-\."
     "\xB7A-Za-z0-9\xC0-\xD6\xD8-\xF6\xF8-\xFF\u0100-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\u0300-\u036F\u203F-\u2040]*")
SpecialValuePattern = re_compile("")
SQNamePattern = re_compile(
    "[_A-Za-z\xC0-\xD6\xD8-\xF6\xF8-\xFF\u0100-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD]"
     r"[_\-\."
     "\xB7A-Za-z0-9\xC0-\xD6\xD8-\xF6\xF8-\xFF\u0100-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\u0300-\u036F\u203F-\u2040]*:"
    r"\S+")
UnitPrefixedQNameSubstitutionChar = "\x07" 
...
JsonMemberTypes = {
    ...
    "/documentInfo": dict,
    "/facts": dict,
    "/*:*": (int,bool,str,dict,list,type(None),NoRecursionCheck,CheckPrefix), 
    ...
    "/documentInfo/*:*": (int,float,bool,str,dict,list,type(None),NoRecursionCheck,CheckPrefix), 
    ...
    "/facts/*/*:*": (int,float,bool,str,dict,list,type(None),NoRecursionCheck,CheckPrefix), 
    }
JsonRequiredMembers = {
    "/": {"documentInfo"},
    "/documentInfo/": {"documentType","taxonomy"},
...
CsvMemberTypes = {
    ...
    "/decimals": (int,str),
    "/links": dict,
    "/*:*": (int,float,bool,str,dict,list,type(None),NoRecursionCheck,CheckPrefix), 
    ...
    "/tableTemplates/*:*": (int,float,bool,str,dict,list,type(None),NoRecursionCheck,CheckPrefix), 
    ...
    "/tableTemplates/*/columns/*/*:*": (int,float,bool,str,dict,list,type(None),NoRecursionCheck,CheckPrefix), 
    ...
    "/tables/*/*:*": (int,float,bool,str,dict,list,type(None),NoRecursionCheck,CheckPrefix), 
...
def parseMetadataCellValues(metadataTable):
    for dimName in metadataTable.keys():
        ...
        "cellValue"
        if dimValue is None or dimValue == "":
            metadataTable[dimName] = None
        elif dimValue == "" and dimName != "period": 
...
def csvPeriod(cellValue, startOrEnd=None):
    ...
    for perType, perFormMatch in periodForms:
        ...
        if m:
            try:
                if perType == PER_ISO:
                    if not m.group(4) and startOrEnd: 
...
def oimEquivalentFacts(f1, f2):
        ...
        if not f1.context.isEqualTo(f2.context):
            ...
            if f1.concept.isLanguage and f2.concept.isLanguage and f1.xValue is not None and f2.xValue is not None:
                return f1.xValue.lower() == f2.xValue.lower() 
            if isinstance(f1.xValue, DateTime): 
                return f1.xValue.dateOnly == f2.xValue.dateOnly and f1.xValue == f2.xValue
            return f1.xValue == f2.xValue 
...
def checkForDuplicates(modelXbrl, allowedDups, footnoteIDs):
    ...
    if allowedDups != ALL:
        ...
        aspectEqualFacts = defaultdict(dict) 
        decVals = {}
        for hashEquivalentFacts in factForConceptContextUnitHash.values():
            if len(hashEquivalentFacts) > 1:
                ...
                for cuDict in aspectEqualFacts.values(): 
                        ...
                        if len(fList) > 1:
                            ...
                            if allowedDups == NONE:
                                ...
                                if any(f.isNil for f in fList):
                                    ...
                                    _v = f0.xValue
                                    _inConsistent = isnan(_v) 
                                    decVals[_d] = _v
                                    aMax, bMin = rangeValue(_v, _d)
                                    for f in fList[1:]:
                                        _d = inferredDecimals(f)
                                        _v = f.xValue
                                        if isnan(_v) or _inConsistent: 
                                            _inConsistent = True
                                            break
                                        if _d in decVals:
        ...
        aspectEqualFootnotes = defaultdict(list) 
...
def loadFromOIM(cntlr, error, warning, modelXbrl, oimFile, mappedUri):
    ...
    try:
        ...
        def openCsvReader(csvFilePath, fileType):
            ...
            if CSV_HAS_HEADER_ROW:
                try:
                    ...
                    _dialect = csv.Sniffer().sniff(chars, delimiters=[',', '\t', ';', '|']) 
        ...
        extensionProperties = OrderedDict() 

        def loadOimObject(oimFile, extendingFile, visitedFiles, primaryReportParameters=None): 
            ...
            if modelXbrl.fileSource.isInArchive(mappedUrl):
                ...
                filepath = modelXbrl.modelManager.cntlr.webCache.getfilename(mappedUrl) "checkModifiedTime",False))
                if filepath:
            ...
            if not isXL:
                ...
                for row in range(1,10): 
                    for col in range(1,10):
                        ...
                        if _metadata and JSONmetadataPattern.match(_metadata): 
                            _foundMatch = True
                            break
                    if _foundMatch:
                        break
            ...
            def checkMemberTypes(obj, path, pathParts):
                if (isinstance(obj,dict)):
                    ...
                    for mbrName, mbrObj in obj.items():
                        ...
                        "mbrName {} mbrObj {}".format(mbrName, mbrObj))
                ...
                if modelXbrl.fileSource.exists(parameterFilePath):
                    ...
                    for i, row in enumerate(openCsvReader(parameterFilePath, CSV_PARAMETER_FILE)):
                        if i == 0:
                            ...
                            "xbrlce:illegalReportParameterRedefinition",
                            "Report parameter %(name)s redefined in file %(file)s, report value %(value1)s, csv value %(value2)s"),
                            ...
                            if objectName in csvExtensibleObjects:
                                ...
                                for extProp, extPropValue in extendedParent.get(objectName,EMPTY_DICT).items():
                                    if extProp in parent.get(objectName,EMPTY_DICT):
                                        if json.dumps(extPropValue,sort_keys=True) != json.dumps(parent[objectName][extProp],sort_keys=True): 
        ...
        if isJSON:
            ...
            footnotes = oimObject.get("facts",{}).values() 
            ...
            reportParameters = oimObject.get("parameters", {}) 
            ...
            def csvFacts():
                ...
                for tableId, table in tables.items():
                    ...
                    try: 
                        ...
                        potentialInvalidReferenceTargets = {} 
                        for rowIndex, row in enumerate(_rowIterator):
                            if rowIndex == 0:
                                ...
                                "xbrlce:invalidPropertyGroupColumnReference",
                                "Table %(table)s propertyFrom %(propFromColumns)s column missing, url: %(url)s"),
                                ", ".join(sorted(missingPropFromCols)), url=tableUrl)
                                ...
                                for commentCol in commentColumns:
                                    colNameIndex.pop(commentCol,None) 
                                ...
                                if isXL and all(cell.value in (None, "") for cell in row): 
                                    continue
                                rowPropGroups = {} 
                                rowPropGroupsUsed = set() 
                                hasRowError = False
                                    ...
                                    if propGrpColIndex < len(row):
                                        ...
                                        if propGrpColValue is NONE_CELL:
                                            error("xbrlce:illegalUseOfNone",
                                                  _("Table %(table)s row %(row)s column %(column)s must not have "),
                                ...
                                for colIndex, colValue in enumerate(row):
                                    ...
                                    factId = "{}.r_{}.{}".format(tableId, rowId or rowIndex, colName) 
                                    fact = {}
                                    
                                    cellValue = _cellValue(colValue) ""
                                    if cellValue is EMPTY_CELL: 
                                        continue
                                                ...
                                                if isinstance(dimValue, str) and dimValue.startswith("$"):
                                                    ...
                                                    if not dimValue.startswith("$"):
                                                        ...
                                                        if paramName == "rowNumber":
                                                            ...
                                                            if dimValue is EMPTY_CELL or dimValue is NONE_CELL: 
                                                                dimValue = NONE_CELL
                                                            else:
                                    ...
                                    if dimValue is not None:
                                        ...
                                        if isinstance(dimValue, str) and dimValue.startswith("$"):
                                            ...
                                            if paramName in colNameIndex:
                                                ...
                                                dimValue = _cellValue(row[colNameIndex[paramName]])
                                                validCsvCell = integerPattern.match(dimValue or "") is not None 
                                                if dimValue is not NONE_CELL and dimValue != "" and dimValue != "":
                                                    factDimensionSourceCol["decimals"] = paramName
                                            elif paramName in tableParameters:
                                                dimSource += " from table parameter " + paramName
                                                dimValue = tableParameters[paramName]
                                                if dimValue != "" and dimValue != "" and integerPattern.match(dimValue):
                                        ...
                                        elif dimValue is not NONE_CELL and dimValue != "" and dimValue != "":
        ...
        if isCSVorXL:
            ...
            def checkDim(tblTmpl, dimName, dimValue, *pathSegs):
                if dimValue is not None:
                    if isinstance(dimValue,str) and dimValue.startswith("") and not SpecialValuePattern.match(dimValue):
                    ...
                    elif dimValue == "" and ":" not in dimName and dimName not in ("concept", "period", "value", "entity", "unit"):
                        ...
                        if dimName == "concept":
                            if dimValue != "":
                                if not isinstance(dimValue,str) or ":" not in dimValue or not QNamePattern.match(dimValue): 
                            ...
                            elif dimValue != "" and not UnitPattern.match( PrefixedQName.sub(UnitPrefixedQNameSubstitutionChar, dimValue) ):
                            ...
                            if dimValue != "" and not PeriodPattern.match(csvPeriod(dimValue) or ""):
                                error("xbrlce:invalidPeriodRepresentation",
                            ...
                            if dimValue != "" and not languagePattern.match(dimValue or ""):
                            ...
                            if dimValue != "" and not isinstance(dimValue,int) and not integerPattern.match(str(dimValue) or ""):
                                error("xbrlce:invalidDecimalsValue",
        ...
        factNum = 0 
        if isJSON:
            syntheticFactFormat = "_f{{:0{}}}".format(int(log10(len(factItems) or 1))) 
        else:
            syntheticFactFormat = "_f{}" 
        ...
        for id, fact in factItems:
            ...
            if isCSVorXL and (not isinstance(conceptSQName,str) or ":" not in conceptSQName or not QNamePattern.match(conceptSQName or "")): 
                error("xbrlce:invalidConceptQName",
            ...
            if ((concept.instanceOfType(UNSUPPORTED_DATA_TYPES) and not concept.instanceOfType(dtrSQNameNamesItemTypes))
                ...
                if concept.periodType == "instant" and _periodType == "forever":
                    ...
                    try:
                        ...
                        if period == "forever":
                            ...
                            endDateTime = dateTime(_end, type=DATETIME)
                        numFactCreationXbrlErrors -= len(modelXbrl.errors) 
                        prevErrLen = len(modelXbrl.errors)
                ...
                if concept.isNumeric:
                    if isCSVorXL and isinstance(text, str): 
            ...
            if concept.baseXbrliType == "QNameItemType": 
        ...
        footnoteLinks = OrderedDict() 
        factLocs = {} 
        footnoteLinkNotes = defaultdict(set) 
        ...
        "Completed in {0:.2} secs".format(time.time() - startedAt),
        "loadFromExcel:info")
...
def isOimLoadable(modelXbrl, mappedUri, normalizedUri, filepath, **kwargs):
    ...
    elif isHttpUrl(normalizedUri) and '?' in _ext: 
...
def validateFinally(val, *args, **kwargs):
    ...
    if getattr(modelXbrl, "loadedFromOIM", False):
        ...
        if fractionFacts:
            modelXbrl.error("xbrlxe:unsupportedFraction", 
        ...
        unsupportedExtRoleRefs = defaultdict(list) 
        footnoteELRs = set()
