...
def _make_chroot(make_vars, bind_paths=None, alpine_version='3.16.0', overlay_backend=None):
    ...
    with _overlay_folders([cache_path], backend=overlay_backend) as path:
        "repopulate" the overlay in order to get a working
...
class KernelTree(Loggable, SerializeViaConstructor):
    ...
    "musttail" return attribute was introduced.
    ...
    def _resolve_toolchain(cls, abi, make_vars, build_env):
        ...
        commands = {
            "musttail" return attribute
            
            **{
                cc: test_cmd(cc)
                "clang" name first in case it's good enough
                for cc in ['clang'] + [
                    f'clang-{i}'
...
class DynamicKmod(Loggable):
    ...
    def _do_compile(self):
        ...
        "bare" kernel tree, i.e. before calling __enter__().
