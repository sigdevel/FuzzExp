...
class YoutubeDL(object):
    ...
    def _outtmpl_expandpath(outtmpl):
        ...
        "%(title)s.%(ext)s" and
        "Hello $PATH", we don't want `$PATH` to be expanded.
    ...
    def prepare_outtmpl(self, outtmpl, info_dict, sanitize=False):
        ...
        def create_key(outer_mobj):
            ...
            if fmt[-1] == 'l':  
                ...
                value, fmt = json.dumps(value, default=_dumpjson_default, indent=4 if '
            elif fmt[-1] == 'q':  
                value = map(str, variadic(value) if '
                ...
                value, fmt = unicodedata.normalize(
                    ...
                    'NF%s%s' % ('K' if '+' in flags else '', 'D' if '
                    value), str_fmt
            elif fmt[-1] == 'D':  
                num_fmt, fmt = fmt[:-1].replace('
                value = format_decimal_suffix(value, f'%{num_fmt}f%s' if num_fmt else '%d%s',
                                              factor=1024 if '
            elif fmt[-1] == 'S':  
                value, fmt = filename_sanitizer(initial_field, value, restricted='
            elif fmt[-1] == 'c':
                if value:
                    value = str(value)[0]
                else:
                    fmt = str_fmt
            elif fmt[-1] not in 'rs':  
                value = float_or_none(value)
                if value is None:
                    value, fmt = default, 's'
    ...
    def _fill_common_fields(self, info_dict, is_video=True):
        ...
        if is_video:
            ...
            if 'title' not in info_dict:
                ...
                info_dict['title'] = f'{info_dict["extractor"]} video "id"]}'
