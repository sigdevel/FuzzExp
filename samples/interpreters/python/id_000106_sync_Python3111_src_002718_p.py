  ...
  def test_aggregate(self):
    ...
    with TestPipeline() as p:
      ...
      assert_that(
          ...
          equal_to([
              beam.Row(square=0, big=False, sum=0, positive=False),   
              beam.Row(square=1, big=False, sum=0, positive=False),   
              beam.Row(square=4, big=False, sum=-2, positive=False),  
              beam.Row(square=4, big=True, sum=2, positive=True),     
          ]))
...
class PTransformLabelsTest(unittest.TestCase):
  ...
  def check_label(self, ptransform, expected_label):
    ...
    self.assertEqual(expected_label, re.sub(r'\d{3,}', '
