...
def show_progress(title, percent, max_width=80):
    if sys.stdout.isatty():
        ...
        sys.stdout.write('%s |%s%s| %s%%\r' % (str(title), '
        sys.stdout.flush()
...
class Git(object):
    ...
    def cleanup():
        ...
        pquery([git_cmd, 'checkout', '--detach', 'HEAD'] + ([] if very_verbose else ['-q'])) 
        branches = []
        lines = pquery([git_cmd, 'branch']).strip().splitlines() 
        for line in lines:
    ...
    def discard(clean_files=False):
        info("Discarding local changes in \"%s\"" % os.path.basename(getcwd()))
        pquery([git_cmd, 'reset', 'HEAD'] + ([] if very_verbose else ['-q'])) 
        pquery([git_cmd, 'checkout', '.'] + ([] if very_verbose else ['-q'])) 
        pquery([git_cmd, 'clean', '-fd'] + (['-x'] if clean_files else []) + (['-q'] if very_verbose else ['-q'])) 
    ...
    def checkout(rev, clean=False):
        ...
        for ref in refs: 
            ...
            if m and m.group(2) != "HEAD": 
                if not os.path.exists(os.path.join('.git', 'refs', 'heads', m.group(2))): 
                    branch = m.group(2)
            elif ref != "HEAD":
                branch = ref 
    ...
    def gettags():
        ...
        for ref in refs:
            ...
            if m:
                t = m.group(2)
                if re.match(r'^(.+)\^\{\}$', t): "pointer"
                    t = re.sub(r'\^\{\}$', '', t) "pointer" chars, e.g. some-tag^{}
                    for tag in tags:
                        if tag[1] == t:
                            tags.remove(tag)
...
class Repo(object):
    ...
    def write(self):
        ...
        if up.hostname:
            url = up._replace(netloc=up.hostname + (':'+str(up.port) if up.port else '')).geturl() 
        else:
            url = self.url "urls" as is

        if os.path.isfile(self.lib):
            with open(self.lib) as f:
                lib_repo = Repo.fromurl(f.read().strip())
                if (formaturl(lib_repo.url, 'https') == formaturl(url, 'https') 
                        and (lib_repo.rev == self.rev                           
                             or (lib_repo.rev and self.rev
                                 and lib_repo.rev == self.rev[0:len(lib_repo.rev)]))):  
        ...
        ref = url.rstrip('/') + '/' + (('' if self.is_build else '
...
def formaturl(url, format="default"):
    ...
    if m:
        ...
        if m and m.group(1) == '': "git@github.com:ARMmbed/mbed-os.git"
...
def publish(all_refs=None, msg=None, top=True):
    ...
    if repo.is_local:
        error(
            ...
            "Read more about publishing local repositories here:\nhttps://github.com/ARMmbed/mbed-cli/" % ("Program" if top else "Library", repo.name, repo.path), 1)
    ...
    for lib in repo.libs:
        if os.path.isdir(lib.path) and Repo.isrepo(lib.path):
            ...
            if (not lib.is_local and not lib_repo.is_local and
                formaturl(lib.url, 'https') != formaturl(lib_repo.url, 'https')): 
                with cd(lib.path):
                    gc, msg = lib_repo.can_update(clean, clean_deps)
                if gc:
...
def releases_(detailed=False, unstable=False, recursive=False, prefix='', p_path=None):
    repo = Repo.fromrepo()
    tags = repo.gettags()
    revtags = repo.gettags(repo.rev) if repo.rev and len(tags) else [] 
    ...
    for tag in tags:
        if re.match(regex_rels, tag[1]):
            rels.append(tag[1] + " %s%s" % ('"", " <- current" if tag[1] in revtags else ""))
