...
class RelationshipLHSTest(fixtures.TestBase, testing.AssertsCompiledSQL):
    ...
    def test_collection_class_uselist(self, decl_base):
        class A(decl_base):
            ...
            id: Mapped[int] = mapped_column(primary_key=True)
            data: Mapped[str] = mapped_column()
            bs_list: Mapped[List["B"]] = relationship(  
                viewonly=True
            )
            bs_set: Mapped[Set["B"]] = relationship(  
                viewonly=True
            )
            bs_list_warg: Mapped[List["B"]] = relationship(  
                "B", viewonly=True
            )
            bs_set_warg: Mapped[Set["B"]] = relationship(  
                "B", viewonly=True
            )

            
            b_one_to_one: Mapped["B"] = relationship(  
                viewonly=True
            )

            b_one_to_one_warg: Mapped["B"] = relationship(  
                "B", viewonly=True
            )
    ...
    def test_collection_class_dict_attr_mapped_collection(self, decl_base):
        class A(decl_base):
            ...
            bs: Mapped[KeyFuncDict[str, "B"]] = relationship(  
...
class CompositeTest(fixtures.TestBase, testing.AssertsCompiledSQL):
    ...
    def test_name_cols_by_str(self, decl_base):
        ...
        class User(decl_base):
            ...
            "zip")
            "zip_" for the attribute. not working
...
class GenericMappingQueryTest(AssertsCompiledSQL, fixtures.TestBase):
    """test the Generic support added as part of """
