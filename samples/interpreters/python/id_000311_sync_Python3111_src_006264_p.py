...
"/var/log/traffic_ops/postinstall.log"


"rotate" this file on every run, for some reason. Should we?
"/var/log/traffic_ops/cpan.log"




"/opt/traffic_ops/install/bin/configuration_file.json"
...
class SSLConfig:
	...
	def __init__(self, gen_cert, cfg_map): 
...
def get_config(questions, fname, automatic = False): 
...
def generate_db_conf(qstns, fname, automatic, root): 
	"""
...
def generate_todb_conf(fname, root, conf): 
	"""
...
def generate_ldap_conf(questions, fname, automatic, root): 
	"""
...
def hash_pass(passwd): 
	...
	if sys.version_info.major >= 3 and hasattr(hashlib, 'scrypt'): 
...
class Scrypt:
	def __init__(self, password, salt, cost_factor, block_size_factor, parallelization_factor, key_length):  
	...
	def derive(self):  
		...
		pack_format = '<' + 'L' * int(salt_length / 4)  
	...
	def ROMix(self, block):  
		xored_block = [0] * len(block)  
		variations = [list()] * self.cost_factor  
		variations[0] = block
		index = 1
	...
	def block_mix(self, previous_block):  
...
def generate_users_conf(qstns, fname, auto, root): 
	"""
...
def generate_profiles_dir(questions): 
	"""
...
def generate_openssl_conf(questions, fname, auto): 
	"""
...
def generate_param_conf(qstns, fname, auto, root): 
	"""
...
def sanity_check_config(cfg, automatic): 
	"""
...
def write_encryption_key(aes_key_location): 
	"""
...
def exec_openssl(description, *cmd_args): 
	"""
...
def setup_certificates(conf, root, ops_user, ops_group): 
	"""
...
def generate_cdn_conf(questions, fname, automatic, root): 
	"""
...
def db_connection_string(dbconf): 
	"""
...
def exec_psql(conn_str, query, **args): 
	"""
...
def invoke_db_admin_pl(action, root, tv): 
	"""
...
def setup_database_data(conn_str, user, param_conf, root, postgresTV): 
	"""
...
no_database, 
	...
	try:
		...
		"/opt/traffic_ops/app/conf/production/tv.conf" section for Traffic Vault PostgreSQL backend is optional
	...
	if not no_restart_to:
		...
		try:
			...
			return 1
		"waiting" before reporting success, so
