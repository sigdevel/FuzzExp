...
$new = qq~<a href="$scripturl?virboard=$currentboard;num=$mnum/new
} else {
$new = qq~<a href="$scripturl?num=$mnum/new
}
} else {
$new = '';
}
...
$pages = qq~ <a href="$scripturl?num=$mnum/~ . (!$ttsreverse ? "0
$pages .= qq~ <a href="$scripturl?num=$mnum/~ . (!$ttsreverse ? "$maxmessagedisplay
...
$pages .= qq~ <a href="$scripturl?num=$mnum/~ . (!$ttsreverse ? "$tmpb
$pages .= qq~ <a href="$scripturl?num=$mnum/~ . (!$ttsreverse ? "$j
$pages .= qq~ <a href="$scripturl?num=$mnum/~ . (!$ttsreverse ? "$i
$pages = qq~<br /><span class="small">&
...
$tempbar =~ s/({|<)yabb lastpostlink(}|>)/<a href="$scripturl?num=$mnum\/$mreplies
...
my $bdnopost = !${$uid.$board}{'canpost'} && $subboard{$board} ? "class='nopost' style='background-color:
