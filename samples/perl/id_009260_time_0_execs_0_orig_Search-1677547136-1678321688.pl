...
<td align="left" width="95%" class="round_top_right">&nbsp;<a href="$scripturl?catselect=$catid{$board}"><u>$catname{$board}</u></a> / <a href="$scripturl?board=$board"><u>$boardname{$board}</u></a> / <a href="$scripturl?num=$tnum/$msgnum
...
$notify = qq~$menusep<a href="$scripturl?action=notify3;oldnotify=1;num=$tnum/$msgnum
} else {
$notify = qq~$menusep<a href="$scripturl?action=notify2;oldnotify=1;num=$tnum/$msgnum
}
}
$yymain .= qq~<a href="$scripturl?board=$board;action=post;num=$tnum/$msgnum
}
...
if ($searchtype == 5) { $search = $FORM{'search'} || $INFO{'search'}; @search = ($search); }
$search = &Censor($search);
my @tmpsearch;
if ($searchtype != 5) {
...
&MakeSmileys if $enable_ubbc && $message !~ /
$message = &Censor($message);
$msub = &Censor($msub);
++$counter;
$yysearchmain .= qq~
