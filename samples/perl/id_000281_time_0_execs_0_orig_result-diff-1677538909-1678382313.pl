...
next if /^rapper:/;
if (/^(.*?) (<.*?>) ([<"_].*?) .$/) {
if ($2 eq "<http://www.w3.org/2001/sw/DataAccess/tests/result-set
$node = $1;
$var = $3;
$var =~ s/"//g;
$vars{$node} = $var;
...
$val =~ s/^_:.*/bNode/;
$val =~ s/"@([a-z-]+)/"@\L$1/i;
if ($val =~ /^"(.*?)"\^\^<http:\/\/www.w3.org\/2001\/XMLSchema
$val = ($1 + 0.0)."^^<http://www.w3.org/2001/XMLSchema
}
if ($val =~ /^"(.*?)"\^\^<http:\/\/www.w3.org\/2001\/XMLSchema
$val = ($1 + 0)."^^<http://www.w3.org/2001/XMLSchema
}
$vals{$sub} = $val;
}
if ($2 eq "<http://www.w3.org/2001/sw/DataAccess/tests/result-set
